...
export default function KioskPage() {
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      const result = await submitTriage({ vitals, symptoms, visual_flags: visualFlags, patient_name: patientName, language: 'en' });
      ...
      <div className="canvas-narrow" style={{ display: 'grid', gap: 14 }}>
        <div className="panel" style={{ padding: 20 }}>
          ...
          <div style={{ marginTop: 20 }}>
            <label className="form-label" style={{ display: 'block', marginBottom: 6 }}>Patient Name (Required)</label>
            <input 
              type="text" 
        ...
        {step === 3 ? <ReviewPanel vitals={vitals} symptoms={symptoms} visualFlags={visualFlags} patientName={patientName} /> : null}
...
function ReviewPanel({ vitals, symptoms, visualFlags, patientName }) {
  return (
    <div className="panel" style={{ padding: 20 }}>
      <h3 style={{ fontSize: 18, marginBottom: 10 }}>Review {patientName ? `- ${patientName}` : ''}</h3>
